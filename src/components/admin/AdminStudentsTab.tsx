      ...
      <div className="bg-white p-5 rounded-3xl border border-stone-200 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onExportCsv}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition shadow-sm cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs bg-stone-50 border border-stone-200 rounded-xl outline-none focus:border-blue-600 focus:bg-white transition"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 text-xs bg-stone-50 border border-stone-200 rounded-xl outline-none focus:border-blue-600 text-slate-700 font-medium"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-stone-50 text-slate-500 uppercase font-black tracking-wider text-[10px] border-b border-stone-200">
              <tr>
                <th className="px-4 py-3 font-poppins">ID</th>
                <th className="px-4 py-3">विद्यार्थी नाव</th>
                <th className="px-4 py-3">मोबाईल / WhatsApp</th>
                <th className="px-4 py-3">तारीख व वेळ</th>
                <th className="px-4 py-3">पेमेंट स्थिती</th>
                <th className="px-4 py-3">Payment ID</th>
                <th className="px-4 py-3 text-center">Quick Messages</th>
              ...
              {filteredStudents.length === 0 ? (
                ...
                filteredStudents.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50/80 transition">
                    ...
                    {/* 1. ID */}
                    <td className="px-4 py-3.5 font-bold font-poppins text-slate-700">
                      {s.id}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      <div className="text-[10px] text-slate-500">{s.occupation} • {s.district}</div>
                      <div className="text-[10px] text-slate-400 truncate max-w-[150px]">{s.email}</div>
                    </td>

                    {/* 3. Mobile & WhatsApp */}
                    <td className="px-4 py-3.5 font-mono">
                      <div className="font-bold text-slate-800">{s.mobileNumber}</div>
                      {s.whatsappNumber !== s.mobileNumber && (
                        <div className="text-[10px] text-emerald-600">WA: {s.whatsappNumber}</div>
                      )}
                    </td>

                    {/* 4. Date & Slot */}
                    <td className="px-4 py-3.5">
                      <div className="font-bold text-slate-800">{s.courseDateDisplay}</div>
                      <div className="text-[10px] text-slate-500 font-medium">{s.slotTimeDisplay}</div>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-800 transition cursor-pointer"
                          title="Delete Record"
                        >
